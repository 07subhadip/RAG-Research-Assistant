...
class RAGService:
    ...
    def initialize(self):
        ...
        if os.path.exists(settings.RAG_DATA_PATH) and any(f.endswith(".pdf") for f in os.listdir(settings.RAG_DATA_PATH)):
    ...
    async def save_files(self, files: List[UploadFile]) -> List[str]:
        ...
        # Validate count
        current_files = [f for f in os.listdir(settings.RAG_DATA_PATH) if f.endswith('.pdf')] if os.path.exists(settings.RAG_DATA_PATH) else []
        ...
        # Check total limit
        existing_pdfs = [f for f in os.listdir(settings.RAG_DATA_PATH) if f.endswith(".pdf")]
        if len(existing_pdfs) + len(files) > settings.MAX_FILES:
            # Maybe user wants to replace? 
            # For now, strict error.
             raise HTTPException(status_code=400, detail=f"Cannot upload. Limit is {settings.MAX_FILES} PDFs total. You have {len(existing_pdfs)}.")
    ...
    async def chat(self, query: str):
        ...
        if not self.pipeline or not self.pipeline.rag_chain:
            raise HTTPException(status_code=400, detail="RAG system not initialized. Upload PDFs first.")
