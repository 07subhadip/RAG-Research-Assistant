...
async def upload_pdfs(files: List[UploadFile] = File(...)):
    if len(files) > settings.MAX_FILES:
        raise HTTPException(status_code=400, detail=f"Maximum {settings.MAX_FILES} PDFs per request.")
    ...
    if rag_service.pipeline:
        current_docs = len(rag_service.pipeline.documents) if rag_service.pipeline.documents else 0
        # If we just look at raw file count:
        if os.path.exists(settings.RAG_DATA_PATH):
           disk_files = len([f for f in os.listdir(settings.RAG_DATA_PATH) if f.endswith('.pdf')])
           if disk_files + len(files) > settings.MAX_FILES:
               raise HTTPException(status_code=400, detail="Total PDF limit reached. Please clear old files via UI or API.")
    ...
    try:
        ...
        return {"message": "Files uploaded and RAG updated successfully", "files": saved_files}
