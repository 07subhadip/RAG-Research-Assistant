...
import { Upload, X, FileText, Loader2, CheckCircle2, Trash2, Database } from 'lucide-react';
...
export function FileUpload({ onUploadSuccess }: FileUploadProps) {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files) {
      ...
      if (serverFiles.length + files.length + pdfs.length > 5) {
        setErrorMsg(`Total limit is 5 files. You have ${serverFiles.length} uploaded.`);
        setStatus('error');
        return;
      }
  ...
  const handleClearSession = async () => {
      if (!confirm("Are you sure you want to delete all files and reset the RAG index?")) return;
  ...
  return (
    <div className="p-4 bg-white dark:bg-gray-900 border-b border-gray-200 dark:border-gray-800">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className={`text-xs ${serverFiles.length + files.length >= 5 ? 'text-orange-500 font-bold' : 'text-gray-500'}`}>
      ...
      {serverFiles.length > 0 && (
          <div className="mb-4 space-y-2">
              <div className="text-xs font-medium text-gray-500 uppercase tracking-tighter mb-1">Indexed Files</div>
              {serverFiles.map((fname, i) => (
                <div key={i} className="flex items-center gap-2 p-2 bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30 rounded text-xs text-blue-700 dark:text-blue-300">
      ...
      {files.length > 0 && (
        <div className="space-y-2">
           <div className="text-xs font-medium text-gray-500 uppercase tracking-tighter">Ready to Index</div>
          {files.map((file, i) => (
            <div key={i} className="flex items-center justify-between p-2 bg-gray-100 dark:bg-gray-800 rounded text-xs group">
          ...
          >
            {isUploading ? <Loader2 className="w-3 h-3 animate-spin" /> : 'Index New Files'}
          </button>
        </div>
      )}
